...
test('Create SCL feature value', async ({ page }, testInfo) => {
    ...
    scl_familyCode = await stepSCLFeature.navigateTo_create_SCL_feature_family(classCode,familyCode);
    
    //SCL feature value
    await helper.assertElementVisible(homePage.genericDict, 'genericDict');
    salesCode = await stepSTVFeature.navigateTo_create_STV_sales_code_new();
    stepHomepage.close_opened_feature_value();
    console.log(salesCode,"stv");
   
    await helper.assertElementVisible(homePage.genericDict, 'genericDict');
    await helper.clickElement(homePage.genericDict, 'genericDict');
   
    featureValue = await stepWPSFeature.navigateTo_create_WPC_feature_new(testData_WPC_FF.testData.category_WPS_options) as string;
    console.log(featureValue,"wpc");
    stepHomepage.close_opened_feature_value();
   
    scl_familyValue = await stepSCLFeature.navigateTo_create_SCL_feature_value(salesCode,featureValue); 
